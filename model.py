...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    loss = K.switch(tf.equal(tf.reduce_sum(pred_active),0.), tf.constant(0.0), tf.reduce_sum(loss) / (tf.reduce_sum(pred_active) + 1))
    return loss
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    skip = tf.reduce_all(tf.cast(tf.greater(tf.size(y_true), 0),tf.bool))
    loss2 = K.switch(skip,
    ...
    return K.switch(tf.is_nan(loss2),tf.zeros([1,1]),loss2 + 0.1*loss1)
    #return loss2


def loss_IoU(pred, gt):
    inter = tf.reduce_sum(tf.multiply(pred, gt))
    union = tf.add(tf.reduce_sum(tf.square(pred)), tf.reduce_sum(tf.square(gt)))
    return K.switch(tf.equal(inter, 0), tf.constant(0.0), 1 - (2 * (inter + 1) / (union + 1)))


def sobel_filter(num_classes):
    fx_part = np.reshape(np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]).astype(np.float32), (3, 3, 1))
    fy_part = np.reshape(np.array([[-1, -2, -1], [0, 0, 0], [1, 2, 1]]).astype(np.float32), (3, 3, 1))

    fx = fx_part
    fy = fy_part
    for i in range(1, num_classes):
        fx = np.concatenate((fx, fx_part), axis=2)
        fy = np.concatenate((fy, fy_part), axis=2)

    fx = np.reshape(fx, (3, 3, num_classes, 1))
    fy = np.reshape(fy, (3, 3, num_classes, 1))

    tf_fx = tf.Variable(tf.constant(fx), name='tf_fx', trainable=False)
    tf_fy = tf.Variable(tf.constant(fy), name='tf_fy', trainable=False)
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    augment: If true, apply random image augmentation. Currently, only
        horizontal flipping is offered.mrcnn_boundary
...
def im_gradient(im, sobel_fx, sobel_fy):
    padding_values = [[0, 0], [1, 1], [1, 1], [0, 0]]
    padded_im = tf.pad(im, padding_values, 'SYMMETRIC')
    stride = [1, 1, 1, 1]
    gx = tf.nn.depthwise_conv2d(padded_im, sobel_fx, stride, padding='VALID')
    gy = tf.nn.depthwise_conv2d(padded_im, sobel_fy, stride, padding='VALID')
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            """boundary_loss = KL.Lambda(lambda x: mrcnn_boundary(*x), name="mrcnn_boundary_loss")(
    ...
    def compile(self, learning_rate, momentum):
        ...
        reg_losses = [keras.regularizers.l2(self.config.WEIGHT_DECAY)(w) for w in self.keras_model.trainable_weights if
